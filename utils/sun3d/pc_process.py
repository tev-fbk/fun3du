...
def crop_extraneous_points_from_point_cloud(
    pcd, dbscan_eps=0.02, dbscan_min_points=10, return_bbox=False, print_debug=False
):

    labels = np.array(
        pcd.cluster_dbscan(
            eps=dbscan_eps, min_points=dbscan_min_points, print_progress=print_debug
        )
    )
...
def crop_point_cloud(pcd, bbox):
    ...
    mask = np.logical_and(
        np.all(point_cloud_np >= bbox.get_min_bound(), axis=1),
