...
if __name__ == "__main__":
    ...
    if args.split == "train_val_set":
        video_id_csv = f"data/scenefun3d/benchmark_file_lists/train_val_set.csv"
    elif args.split == "train_val_set" and args.download_only_one_video_sequence:
        video_id_csv = (
            f"data/scenefun3d/benchmark_file_lists/train_val_set_only_one_video.csv"
        )
    elif args.split == "test_set":
        video_id_csv = f"data/scenefun3d/benchmark_file_lists/test_set.csv"
    elif args.split == "test_set" and args.download_only_one_video_sequence:
        video_id_csv = (
            f"data/scenefun3d/benchmark_file_lists/test_set_only_one_video.csv"
        )
        ...
        if args.video_id_csv:
            ...
            df = pd.DataFrame(
                {"visit_id": [args.visit_id], "video_id": [args.video_id]}
            )
        else:
            assert (
                False
            ), "For split 'custom', either a) video_id_csv must be provided or b) visit_id and video_id must be provided"
    ...
    for index, row in df.iterrows():
        ...
        print(
            f"[*] Downloading assets - visit_id: {visit_id}, video_id: {video_id} (progress: {index} / {number_of_rows})"
        )
        ...
        if visit_dataset_assets and str(visit_id) not in prev_visit_id_list:
            download_assets_for_visit_id(
                str(visit_id), current_asset_download_dir, split, visit_dataset_assets
            )
