...
from . import util, util_3d
from .benchmark_labels import CLASS_LABELS, VALID_CLASS_IDS
...
def compute_averages(aps, factor=100):  # factor is 1 or 100 (for percent)
    ...
    for (li, label_name) in enumerate(CLASS_LABELS):
        ...
        avg_dict["classes"][label_name]["ap"] = (
            np.average(aps[d_inf, li, oAllBut25]) * factor
        )
        avg_dict["classes"][label_name]["ap50%"] = (
            np.average(aps[d_inf, li, o50]) * factor
        )
        avg_dict["classes"][label_name]["ap25%"] = (
            np.average(aps[d_inf, li, o25]) * factor
        )
    return avg_dict
...
def fast_hist(pred, gt, num_labels):
    ...
    return np.bincount(
        num_labels * gt[k].astype(int) + pred[k], minlength=num_labels**2
